...
def add_regularization(model, reg_method, param):
    ...
    {1: add_combined_reg(model, param),
     2: add_separate_reg(model, param),
     3: sanity_check}[reg_method]
...
def sanity_check(model, param):
    ...
    for j in range(1, model.nblayer):
        ...
        feature_sum = feature_sum.sum(0)
        feature_sum = (
        feature_sum.view_as(weight.data))  # expand the feature sum into a tensor with same dimensions as BN weights
