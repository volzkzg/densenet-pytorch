...
def main():
    ...
    original_cifar10 = datasets.CIFAR10('../data', train=True, download=True, transform=transform_train)
    len_train = len(original_cifar10)
    list = range(0, len_train)
    slice = random.sample(list, len_train/2)
    new_cifar10 = [original_cifar10[i] for i in slice]                      # half the dataset
    # print  new_cifar10
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss.backward()
        ############################################################################################
        add_regularization(model, 5 * 1e-5)
        ############################################################################################
        optimizer.step()
...
def add_regularization(model, reg_param):
    for i in range(model.nblayer):
        ...
        for j in range(len(weight.grad.data) - 2, -1, -1):
            # reg *= decay
            reg = reg * (j + 1) / len(weight.grad.data)             # This is the linear decay
